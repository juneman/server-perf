		...
		u_int limit, io_msg_s *iomsg) 
{
	...
	while (m < limit) {
		...
		if (rs->len < 14 || rs->len > 2048)
			...
			D("send len %d rx[%d]", rs->len, j);

		pkt_copy(rxbuf + 14 + 20 + 8 , iomsg->buff, rs->len - 14 - 20 - 8);
        iomsg->n = rs->len;
NEXT_L:	
		...
		u_int limit, io_msg_s *iomsg)
{
	...
	while (m < limit) {
		...
		ts->len = iomsg->buff_len;
		pkt_copy(iomsg->buff, txbuf, ts->len);
...
int netmap_send_to_ring(struct my_ring *src)
{
    int limit = 512;
	struct netmap_ring *txring;
	...
	while (ti < src->end) {
		...
		m += process_send_ring(txring, limit);
        break;
	}
