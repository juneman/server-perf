...
int netmap_recv(int fd, char *buff, int buff_len, netmap_address_t *addr) 
{
    ...
    int n = lfqueue_dequeue(&(line->recv_queue), buff, buff_len, (char*)addr, sizeof(netmap_address_t));
    if (n <= 0) 
    {
...
LOOP:
    {
        ...
        { 
            data.len = recv_bytes;
            if (lfqueue_enqueue(&(line->recv_queue), 
                    data.buff, data.len, data.extbuff, sizeof(netmap_address_t)) <= 0 )
                goto END;

            have_data = NM_R_SUCCESS;
            node->recv_count ++;
            
            if (NM_R_SUCCESS == have_data)
