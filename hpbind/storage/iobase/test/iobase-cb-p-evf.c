...
CREATE_PIPE_LINE:    
    ...
    line->nmfd = nmfd;
    line->actived = 1;
    g_iomgr.pipeline_map[line->efd] = line;
...
//
        ...
        g_iomgr.pipeline_map[line->efd] = NULL;
    }
...
int netmap_recv(int fd, char *buff, int buff_len, netmap_address_t *addr) 
{
    ...
    uint64_t buf;
    read(line->efd, &buf, sizeof(uint64_t));
...
static int __netmap_signal__(int nmfd)
{
    ...
    netmap_pipeline_t * line = NULL;
    static uint64_t _buff_ = 1;
	
    line = netmap_ifnode_next_line_by_nmfd(&g_iomgr, nmfd);
    if (NULL != line)
    {
        ++_buff_;
        write(line->efd, &_buff_, sizeof(uint64_t)); 
