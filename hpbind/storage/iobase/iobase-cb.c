...
#define NM_LIB "CBUFF-LOCK-LIB"
#else
#define NM_LIB "CBUFF-LF-LIB"
#endif
...
int iobase_recv(int fd, char *buff, int buff_len, iobase_address_t *addr) 
{
    ...
    int n = cbuff_read(&(line->recv_buff), 
                        buff, buff_len, 
                        (char*)addr, sizeof(iobase_address_t));
    if (n <= 0) 
    {
...
int iobase_send(int fd, const char *buff, int data_len, const iobase_address_t *addr)
{
    ...
    int n = cbuff_write(&(line->send_buff), 
                    buff, data_len, 
                    (char*)addr, sizeof(iobase_address_t));
    if (n <= 0)
    {
